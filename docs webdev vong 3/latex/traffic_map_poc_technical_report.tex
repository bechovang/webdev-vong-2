...
9 & Dữ liệu thời gian thực & Tích hợp TomTom Traffic API, điều chỉnh dự báo XGBoost bằng dữ liệu real-time & Trung bình \\[4pt]
10 & Giao diện responsive & Hỗ trợ cả desktop và mobile, panel thu gọn, touch-friendly & Cao \\
\bottomrule
\end{tabularx}
\end{table}
...
9 & Trợ lý AI Chatbot & Hoàn thành & 8 intents tiếng Việt, LLM enhancement, action system \\[4pt]
10 & Giao diện responsive & Hoàn thành & Desktop + mobile, collapsible panels, touch-friendly \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item Dữ liệu thực: Road segments và traffic history từ Sở GTVT TP. Hồ Chí Minh, không phải dữ liệu giả.
...
SmartRoute có tiềm năng ứng dụng thực tế cao trong nhiều bối cảnh:
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Người dùng cá nhân (B2C):} Sinh viên, nhân viên văn phòng tại TP. Hồ Chí Minh cần dự báo giao thông để lên kế hoạch di chuyển.
    \item \textbf{Doanh nghiệp giao hàng (B2B):} Logistics, food delivery, ride-hailing cần tối ưu lộ trình dựa trên dự báo ùn tắc để giảm chi phí và tăng SLA.
    \item \textbf{Cơ quan quản lý (B2G):} Sở GTVT, CSGT cần dữ liệu phân tích giao thông theo khu vực và thời gian.
\end{itemize}

\subsection{Hướng phát triển}
Các hướng phát triển tiếp theo cho SmartRoute:
\begin{itemize}[leftmargin=1.2cm]
    \item Mở rộng dữ liệu: Thu thập dữ liệu giao thông mới hơn, bổ sung dữ liệu thời tiết, sự kiện.
    \item Nâng cấp ML: Thử nghiệm LSTM cho time-series forecasting, thu thập thêm dữ liệu training.
    \item Self-hosted services: Valhalla routing, Pelias geocoding, vector tiles từ OSM data.
    \item Mobile app: Phát triển native mobile app với React Native hoặc Flutter.
    \item User accounts: Lưu lịch sử route, preferred routes, personalized commute alerts.
    \item Push notifications: Cảnh báo chủ động khi phát hiện nguy cơ kẹt xe trên lộ trình quen.
    \item PostgreSQL/PostGIS: Migrate từ SQLite sang PostgreSQL với PostGIS cho spatial queries mạnh mẽ hơn.
    \item WebSocket: Real-time traffic updates thay vì polling.
\end{itemize}
