...
\begin{tikzpicture}[
    ui/.style={draw, rounded corners, align=center, minimum width=4cm, minimum height=1cm, fill=blue!8},
    api/.style={draw, rounded corners, align=center, minimum width=4cm, minimum height=1cm, fill=green!8},
    ext/.style={draw, rounded corners, align=center, minimum width=3.8cm, minimum height=1cm, fill=orange!10},
    data/.style={draw, rounded corners, align=center, minimum width=4.2cm, minimum height=1cm, fill=gray!12},
    ml/.style={draw, rounded corners, align=center, minimum width=4cm, minimum height=1cm, fill=purple!10},
...
% Row 2 - Frontend
\node[ui, minimum width=7cm] at (0, -2) (fe) {Next.js Client UI\\MapView, SearchBox, RouteLayer, Chat};

% Row 3 - BFF Layer
\node[api] at (-4.5, -4.2) (bff1) {BFF API Routes\\\texttt{/api/segments-hcmc}};
\node[api] at (4.5, -4.2) (bff2) {BFF API Routes\\\texttt{/api/route, /api/chat}};

% Row 4 - Backend (tăng khoảng cách x)
\node[ml] at (-5.5, -6.4) (fastapi) {FastAPI Backend\\XGBoost Prediction};
\node[data] at (0, -6.4) (db) {SQLite Database\\84k segments, 577k nodes};
\node[api] at (5.5, -6.4) (bff3) {BFF Route Alt.\\\texttt{/api/route/alternatives}};
